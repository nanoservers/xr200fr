<?php
// $Id: main.php 10936 2013-01-27 23:53:28Z beckmi $
//  ------------------------------------------------------------------------ //
...
//  Check is active
if ( !xoops_getModuleOption('active_smilies', 'system') ) redirect_header( 'admin.php', 2, _AM_SYSTEM_NOTACTIVE );
	...
	case 'list': default:
		...
		if ( $smilies_count > 0 ){
			foreach (array_keys($smilies_arr) as $i){
				...
				$smilies_img = ($smilies_arr[$i]->getVar("smile_url")) ? $smilies_arr[$i]->getVar("smile_url") : 'blank.gif';

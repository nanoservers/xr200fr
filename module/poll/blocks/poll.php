<?php
// $Id: poll.php,v 1.8 2004/12/26 19:12:12 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
function b_poll_show()
{
	global $xoopsUser;
	$block = array();
	$polls =& poll::getAll(array('display=1'), true, 'weight ASC, end_time DESC');
	$count = count($polls);
	...
	for ($i = 0; $i < $count; $i++) {
		$options_arr =& pollOption::getAllByPollId($polls[$i]->getVar('poll_id'));
		$option_type = 'radio';
		...
		if ( pollLog::hasVoted($polls[$i]->getVar('poll_id'), xoops_getenv('REMOTE_ADDR'),$uid)){
			$hasVoted=1;
		}else{
			$hasVoted=0;
		}
		...
		$poll = array('id' => $polls[$i]->getVar('poll_id'), 'question' => $polls[$i]->getVar('question'), 'option_type' => $option_type, 'option_name' => $option_name, 'options' => $options,'has_expired'=>$polls[$i]->hasExpired(), 'votes' => $polls[$i]->getVar('votes'), 'has_voted'=>$hasVoted, 'totalVotes' => sprintf(_PL_TOTALVOTES, $totalVotes), 'comments' => poll::getcomments($polls[$i]->getVar('poll_id')), 'end_time'=>formatTimeStamp($polls[$i]->getVar('end_time'), "m"), 'comment_mode'=> pollLog::commentMode());
		$block['polls'][] =& $poll;
		unset($options);
		unset($poll);
	}

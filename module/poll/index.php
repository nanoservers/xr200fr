...
if ( empty($poll_id) ) {
	$xoopsOption['template_main'] = 'poll_index.html';
    ...
    $polls['pollId'] = $polls_arr[$i]->getVar("poll_id");
		if ( $polls_arr[$i]->getVar("end_time") > time() ) {
			...
			if ( pollLog::hasVoted($polls_arr[$i]->getVar('poll_id'), xoops_getenv('REMOTE_ADDR'),$uid)){
	...
	if ( !$poll->hasExpired() ) {
		...
		if ( empty($voted_polls[$poll_id]) ) {
			if ( $xoopsUser ) {
				if ( pollLog::hasVoted($poll_id, xoops_getenv('REMOTE_ADDR'), $xoopsUser->getVar("uid")) ) {
				...
				if ( pollLog::hasVoted($poll_id, xoops_getenv('REMOTE_ADDR')) ) {
					setcookie("voted_polls[$poll_id]", 1, 0);
					$msg = _PL_ALREADYVOTED;
				} else {
		...
		if ( $poll->getVar("mail_status") != POLL_MAILED ) {
			...
			$xoopsMailer->useMail();
			$xoopsMailer->setTemplateDir(XOOPS_ROOT_PATH."/modules/poll/language/".$xoopsConfig['language']."/mail_template/");
	...
	if ($url != null){
		...
		redirect_header(XOOPS_URL."/modules/poll/pollresults.php?poll_id=$poll_id", 1, $msg);
	}

	exit();
} elseif ( !empty($poll_id) ) {
	$xoopsOption['template_main'] = 'poll_view.html';
	include XOOPS_ROOT_PATH."/header.php";
	$poll = new poll($poll_id);

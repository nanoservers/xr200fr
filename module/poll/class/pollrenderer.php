<?php
// $Id: pollrenderer.php,v 1.11 2004/12/26 19:12:12 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
// ------------------------------------------------------------------------- //
include_once XOOPS_ROOT_PATH."/modules/poll/language/".$xoopsConfig['language']."/main.php";
...
class pollRenderer
{
	...
	function renderForm()
	{
		$content = "<form action='".XOOPS_URL."/modules/poll/index.php' method='post'>";
		...
		$options_arr =& pollOption::getAllByPollId($this->poll->getVar("poll_id"));
		$option_type = "radio";
		...
		foreach ( $options_arr as $option ) {
        	...
        	$content .= "<input type='button' value='"._PL_RESULTS."' class='button' onclick='location=\"".XOOPS_URL."/modules/poll/pollresults.php?poll_id=".$this->poll->getVar("poll_id")."\"' />";
    ...
    function assignForm(&$tpl)
    {
		...
		$tpl->assign('poll', array('question' => $this->poll->getVar("question"), 'pollId' => $this->poll->getVar("poll_id"), 'viewresults' => XOOPS_URL."/modules/poll/pollresults.php?poll_id=".$this->poll->getVar("poll_id"), 'action' => XOOPS_URL."/modules/poll/index.php", 'options' => $options, 'description' => $this->poll->getVar("description")));
    }
	...
	function renderResults()
	{
		...
		foreach ( $options_arr as $option ) {
			...
			if ( $percent > 0 ) {
				...
				echo "<img src='".XOOPS_URL."/modules/poll/images/colorbars/".$option->getVar("option_color", "E")."' height='14' width='".$width."' align='middle' alt='".intval($percent)." %' />";
			}
		...
		if ( !$this->poll->hasExpired() ) {
			echo "<br />[<a href='".XOOPS_URL."/modules/poll/index.php?poll_id=".$this->poll->getVar("poll_id")."'>"._PL_VOTE."</a>]";
		}
    ...
    function assignResults(&$tpl)
    {
		...
		foreach ( $options_arr as $option ) {
			...
			if ( $percent > 0 ) {
				$width = intval($percent)*2;
				$options[$i]['image'] = "<img src='".XOOPS_URL."/modules/poll/images/colorbars/".$option->getVar("option_color", "E")."' height='14' width='".$width."' align='middle' alt='".intval($percent)." %' />";
			}
		...
		if ( !$this->poll->hasExpired() && !pollLog::hasVoted($this->poll->getVar('poll_id'), xoops_getenv('REMOTE_ADDR'),$uid)) {
			$vote = "<a href='".XOOPS_URL."/modules/poll/index.php?poll_id=".$this->poll->getVar("poll_id")."'>"._PL_VOTE."</a>";
		} else {
			$vote = null;
		}

		$tpl->assign('poll', array('question' => $this->poll->getVar("question"),'end_text' => $end_text,'totalVotes' => sprintf(_PL_TOTALVOTES, $total), 'totalVoters' => sprintf(_PL_TOTALVOTERS, $this->poll->getVar("voters")),'vote' => $vote, 'options' => $options, 'description' => $this->poll->getVar("description")));
    }
}
?>
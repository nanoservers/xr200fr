<?php
// $Id: polllog.php,v 1.11 2004/12/26 19:12:12 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
class pollLog extends XoopsObject
{
	var $db;

	// constructor
	function pollLog($id=null)
	{
	...
	function store()
	{
		...
		$log_id = $this->db->genId($this->db->prefix("poll_log")."_log_id_seq");
		$sql = "INSERT INTO ".$this->db->prefix("poll_log")." (log_id, poll_id, option_id, ip, user_id, time) VALUES ($log_id, $poll_id, $option_id, ".$this->db->quoteString($ip).", $user_id, ".time().")";
	...
	function load($id)
	{
		$sql = "SELECT * FROM ".$this->db->prefix("poll_log")." WHERE log_id=".$id."";
	...
	function delete()
	{
		$sql = sprintf("DELETE FROM %s WHERE log_id = %u", $this->db->prefix("poll_log"), $this->getVar("log_id"));
...
function hasVoted($poll_id, $ip, $user_id=null)
{
	...
	$module =& $module_handler->getByDirname('poll');
	...
	if ($limit_by_uid != 1 && $limit_by_ip != 1){
		...
		$db =& Database::getInstance();
		$sql = "SELECT COUNT(*) FROM ".$db->prefix("poll_log")." WHERE poll_id=".intval($poll_id).$filter;
	...
	{
		$db =& Database::getInstance();
		$sql = sprintf("DELETE FROM %s WHERE poll_id = %u", $db->prefix("poll_log"), intval($poll_id));
        	if ( !$db->query($sql) ) {
			return false;
		}
		return true;
	...
	{
		$db =& Database::getInstance();
		$sql = sprintf("DELETE FROM %s WHERE option_id = %u", $db->prefix("poll_log"), intval($option_id));
        	if ( !$db->query($sql) ) {
			return false;
		}
		return true;

<?php
// $Id: poll.php,v 1.12 2004/12/26 19:12:12 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
class poll extends XoopsObject
{
	...
	function store()
	{
		...
		if ( empty($poll_id) ) {
			$poll_id = $this->db->genId($this->db->prefix("poll_desc")."_poll_id_seq");
			$sql = "INSERT INTO ".$this->db->prefix("poll_desc")." (poll_id, question, description, user_id, start_time, end_time, votes, voters, display, anonymous, weight, multiple, mail_status) VALUES ($poll_id, ".$this->db->quoteString($question).", ".$this->db->quoteString($description).", $user_id, $start_time, $end_time, 0, 0, $display, $anonymous, $weight, $multiple, $mail_status)";
		} else {
			$sql ="UPDATE ".$this->db->prefix("poll_desc")." SET question=".$this->db->quoteString($question).", description=".$this->db->quoteString($description).", start_time=$start_time, end_time=$end_time, display=$display, anonymous=$anonymous, weight=$weight, multiple=$multiple, mail_status=$mail_status WHERE poll_id=$poll_id";
		}
		//echo $sql;
	...
	function load($id)
	{
		$sql = "SELECT * FROM ".$this->db->prefix("poll_desc")." WHERE poll_id=".$id."";
	...
	function delete()
	{
		$sql = sprintf("DELETE FROM %s WHERE poll_id = %u", $this->db->prefix("poll_desc"), $this->getVar("poll_id"));
	...
	function vote($option_id, $ip, $user_id=null)
	{
		if (!empty($option_id)) {
			if (is_array($option_id)) {
				...
				$option = new pollOption($option_id);
				if ( $this->getVar("poll_id") == $option->getVar("poll_id") ) {
					$log = new pollLog();
					$log->setVar("poll_id", $this->getVar("poll_id"));
	...
	{
		$votes = pollLog::getTotalVotesByPollId($this->getVar("poll_id"));
		$voters = pollLog::getTotalVotersByPollId($this->getVar("poll_id"));
		$sql ="UPDATE ".$this->db->prefix("poll_desc")." SET votes=$votes, voters=$voters WHERE poll_id=".$this->getVar("poll_id")."";
		$this->db->query($sql);
	}
	...
	{ 
        ...
        $pollmod = $module_handler->getByDirname('poll'); 

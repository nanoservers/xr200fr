...
class video_downloads extends XoopsObject
{
    ...
    function getForm($donnee = array(), $form_type, $erreur = false, $action = false)
    {
        ...
        // Set type
        $homepage = new XoopsFormSelect (_AM_VIDEO_TYPE, 'type', $this->getVar("type"));
        $homepage->addOption('ntv', '.NTV');
        $homepage->addOption('mp4', '.MP4');
        $homepage->addOption('flv', '.FLV');
        $fichier->addElement($homepage);
        ...
        if ($xoopsUser->isAdmin($xoopsModule->mid())) {
            $form->addElement(new XoopsFormRadioYN (_AM_VIDEO_TOP, 'top', $this->isNew() ? 0 : $this->getVar('top', 'e')));
        }

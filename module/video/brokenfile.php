...
$xoopsOption ['template_main'] = 'video_brokenfile.html';
...
switch ($op) {
    ...
    case "liste" :
        ...
        $breadcrumb .= ' &raquo; ' . _MD_VIDEO_SINGLEFILE_REPORTBROKEN;
        $xoopsTpl->assign('breadcrumb', $breadcrumb);
        // r�f�rencement
        // titre de la page
        $pagetitle = _MD_VIDEO_SINGLEFILE_REPORTBROKEN . ' - ' . $view_downloads->getVar('title') . ' - ';
        $pagetitle .= video_PathTreeUrl($mytree, $view_downloads->getVar('cid'), $downloadscat_arr, 'cat_title', $prefix = ' - ', false, 'DESC', true);
        $xoopsTpl->assign('xoops_pagetitle', $pagetitle);
        //description
        $xoTheme->addMeta('meta', 'description', strip_tags(_MD_VIDEO_SINGLEFILE_REPORTBROKEN . ' (' . $view_downloads->getVar('title') . ')'));
    ...
    case "save" :
        ...
        if ($ratinguser != 0) {
            ...
            foreach (array_keys($downloadsbroken_arr) as $i) {
                if ($downloadsbroken_arr [$i]->getVar('sender') == $ratinguser) {
                    redirect_header('singlefile.php?lid=' . $lid, 2, _MD_VIDEO_BROKENFILE_ALREADYREPORTED);
                    exit ();
                }
            }
        } else {
        ...
        if ($erreur == true) {
            ...
            if ($downloadsbroken_Handler->insert($obj)) {
                ...
                redirect_header('singlefile.php?lid=' . $lid, 2, _MD_VIDEO_BROKENFILE_THANKSFORINFO);
            }

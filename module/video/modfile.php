...
$xoopsOption ['template_main'] = 'video_modfile.html';
...
switch ($op) {
    ...
    case "list" :
        ...
        $breadcrumb .= ' &raquo; ' . _MD_VIDEO_SINGLEFILE_MODIFY;
        $xoopsTpl->assign('breadcrumb', $breadcrumb);
        // r�f�rencement
        // titre de la page
        $pagetitle = _MD_VIDEO_SINGLEFILE_MODIFY . ' - ' . $view_downloads->getVar('title') . ' - ';
        $pagetitle .= video_PathTreeUrl($mytree, $view_downloads->getVar('cid'), $downloadscat_arr, 'cat_title', $prefix = ' - ', false, 'DESC', true);
        $xoopsTpl->assign('xoops_pagetitle', $pagetitle);
        //description
        $xoTheme->addMeta('meta', 'description', strip_tags(_MD_VIDEO_SINGLEFILE_MODIFY . ' (' . $view_downloads->getVar('title') . ')'));
    ...
    case "save" :
        ...
        if (intval($_REQUEST ['size']) == 0) {
            if ($_REQUEST ['size'] == '0' || $_REQUEST ['size'] == '') {
                ...
                $erreur = true;
                $message_erreur .= _MD_VIDEO_ERREUR_SIZE . '<br>';
            }
        }
        ...
        if (isset ($_REQUEST ['cid'])) {
            if ($_REQUEST ['cid'] == 0) {
                $erreur = true;
                $message_erreur .= _MD_VIDEO_ERREUR_NOCAT . '<br>';
            }
        }
        ...
        if ($erreur == true) {
            ...
            if ($downloadsmod_Handler->insert($obj)) {
                ...
                redirect_header('singlefile.php?lid=' . intval($_REQUEST ['lid']), 1, _MD_VIDEO_MODFILE_THANKSFORINFO);
            }

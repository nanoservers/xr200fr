...
$statut_menu = video_CleanVars($_REQUEST, 'statut_display', 1, 'int');
...
switch ($op) {
    ...
    case "list":
        ...
        if (video_checkModuleAdmin()) {
            ...
            if ($statut_menu == 1) {
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWUPLOAD, 'downloads.php?op=new_downloads&type=upload', 'add');
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWCONVERT, 'downloads.php?op=new_downloads&type=convert', 'add');
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWCOPY, 'downloads.php?op=new_downloads&type=copy', 'add');
                if ($downloads_waiting == 0) {
                    $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_WAIT, 'downloads.php?op=list&statut_display=0', 'add');
                } else {
                    $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_WAIT, 'downloads.php?op=list&statut_display=0', 'add', ' (<span style="color : Red">' . $downloads_waiting . '</span>)');
                }
            } else {
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_LISTE, 'downloads.php?op=list', 'list');
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWUPLOAD, 'downloads.php?op=new_downloads&type=upload', 'add');
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWCONVERT, 'downloads.php?op=new_downloads&type=convert', 'add');
                $downloads_admin->addItemButton(_AM_VIDEO_DOWNLOADS_NEWCOPY, 'downloads.php?op=new_downloads&type=copy', 'add');
            }
        ...
        if ($numrowscat == 0) {
            redirect_header('category.php?op=new_cat', 2, _AM_VIDEO_REDIRECT_NOCAT);
        }
        $criteria = new CriteriaCompo();
        ...
        $start = video_CleanVars($_REQUEST, 'start', 0, 'int');
        $criteria->setStart($start);
        $criteria->setLimit($limit);
        //pour faire une jointure de table
        $downloads_Handler->table_link = $downloads_Handler->db->prefix("video_cat"); // Nom de la table en jointure
        ...
        if ($numrows > 0) {
            echo '<div class="floatleft" align="left" width="49%"><form id="form_document_edit" name="form_document_edit" method="get" action="downloads.php">' . _AM_VIDEO_DOWNLOADS_EDITFORM . ' ';
            ...
            echo _AM_VIDEO_TRIPAR . "<select name=\"document_tri\" id=\"document_tri\" onchange=\"location='" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/admin/downloads.php?statut_display=$statut_display&document_order=$document_order&document_tri='+this.options[this.selectedIndex].value\">";
            echo '<option value="1"' . ($document_tri == 1 ? ' selected="selected"' : '') . '>' . _AM_VIDEO_FORMDATE . '</option>';
            echo '<option value="2"' . ($document_tri == 2 ? ' selected="selected"' : '') . '>' . _AM_VIDEO_FORMTITLE . '</option>';
            echo '<option value="3"' . ($document_tri == 3 ? ' selected="selected"' : '') . '>' . _AM_VIDEO_FORMHITS . '</option>';
            echo '<option value="4"' . ($document_tri == 4 ? ' selected="selected"' : '') . '>' . _AM_VIDEO_FORMRATING . '</option>';
            echo '<option value="5"' . ($document_tri == 5 ? ' selected="selected"' : '') . '>' . _AM_VIDEO_FORMCAT . '</option>';
            echo '</select> ';
            echo _AM_VIDEO_ORDER . "<select name=\"order_tri\" id=\"order_tri\" onchange=\"location='" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/admin/downloads.php?statut_display=$statut_display&document_tri=$document_tri&document_order='+this.options[this.selectedIndex].value\">";
            ...
            echo '<tr>';
            echo '<th align="center" width="5%">' . _AM_VIDEO_FORMFILE . '</th>';
            echo '<th align="left" width="20%">' . _AM_VIDEO_FORMTITLE . '</th>';
            echo '<th align="left">' . _AM_VIDEO_FORMCAT . '</th>';
            echo '<th align="center" width="5%">' . _AM_VIDEO_FORMHITS . '</th>';
            echo '<th align="center" width="5%">' . _AM_VIDEO_FORMRATING . '</th>';
            echo '<th align="center" width="15%">' . _AM_VIDEO_FORMACTION . '</th>';
            echo '</tr>';
            $mytree = new XoopsObjectTree($category_arr, 'cat_cid', 'cat_pid');
            $class = 'odd';
            foreach (array_keys($downloads_arr) as $i) {
                $class = ($class == 'even') ? 'odd' : 'even';
                $category = video_PathTree($mytree, $downloads_arr[$i]->getVar('cid'), $category_arr, 'cat_title', $prefix = ' <img src="../images/deco/arrow.gif"> ');
                echo '<tr class="' . $class . '">';
                ...
                echo '<td align="center">';
                echo($statut_display == 1 ? '<a href="downloads.php?op=lock_status&downloads_lid=' . $i . '"><img src="./../images/icon/lock.png" border="0" alt="' . _AM_VIDEO_FORMLOCK . '" title="' . _AM_VIDEO_FORMLOCK . '"></a> ' : '<a href="downloads.php?op=update_status&downloads_lid=' . $i . '"><img src="./../images/icon/renew_mini.gif" border="0" alt="' . _AM_VIDEO_FORMVALID . '" title="' . _AM_VIDEO_FORMVALID . '"></a> ');
                echo '<a href="downloads.php?op=view_downloads&downloads_lid=' . $i . '"><img src="../images/icon/view_mini.png" alt="' . _AM_VIDEO_FORMDISPLAY . '" title="' . _AM_VIDEO_FORMDISPLAY . '"></a> ';
                echo '<a href="downloads.php?op=edit_downloads&downloads_lid=' . $i . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_VIDEO_FORMEDIT . '" title="' . _AM_VIDEO_FORMEDIT . '"></a> ';
                echo '<a href="downloads.php?op=del_downloads&downloads_lid=' . $i . '"><img src="../images/icon/delete_mini.gif" alt="' . _AM_VIDEO_FORMDEL . '" title="' . _AM_VIDEO_FORMDEL . '"></a>';
                echo '</td>';
            }
            echo '</table><br />';
            echo '<br /><div align=right>' . $pagenav . '</div><br />';
        } else {
            echo '<div class="errorMsg" style="text-align: center;">' . _AM_VIDEO_ERREUR_NODOWNLOADS . '</div>';
        }
    ...
    case "new_downloads":
        ...
        $form_type = video_CleanVars($_REQUEST, 'type', 'upload', 'string');
    ...
    case "edit_downloads":
        ...
        $downloads_lid = video_CleanVars($_REQUEST, 'downloads_lid', 0, 'int');
    ...
    case "del_downloads":
        ...
        if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
            ...
            Video_Updateposts($obj->getVar('submitter'), $obj->getVar('status'), 'delete');
            if ($downloads_Handler->delete($obj)) {
                ...
                redirect_header('downloads.php', 1, _AM_VIDEO_REDIRECT_DELOK);
            } else {
            ...
            xoops_confirm(array('ok' => 1, 'downloads_lid' => $downloads_lid, 'op' => 'del_downloads'), $_SERVER['REQUEST_URI'], sprintf(_AM_VIDEO_FORMSUREDEL, $obj->getVar('title')) . '<br><br>' . _AM_VIDEO_FORMWITHFILE . ' <b><a href="' . $obj->getVar('url') . '">' . $obj->getVar('url') . '</a></b><br>');
        }
        break;
    ...
    case "view_downloads":
        ...
        $category = video_PathTree($mytree, $view_downloads->getVar('cid'), $category_arr, 'cat_title', $prefix = ' <img src="../images/deco/arrow.gif"> ');
        ...
        echo '</tr>';
        echo '<tr class="even">';
        echo '<td width="30%">' . _AM_VIDEO_FORMFILE . ' </td>';
        ...
        echo '</tr>';
        echo '<tr class="odd">';
        echo '<td width="30%">' . _AM_VIDEO_FORMCAT . ' </td>';
        echo '<td>' . $category . '</td>';
        echo '</tr>';
        $criteria = new CriteriaCompo();
            ...
            if ($downloads_field[$i]->getVar('status_def') == 1) {
                if ($downloads_field[$i]->getVar('fid') == 1) {
                    ...
                    if ($view_downloads->getVar('homepage') != '') {
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_VIDEO_FORMHOMEPAGE . ' </td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 2) {
                    ...
                    if ($view_downloads->getVar('version') != '') {
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_VIDEO_FORMVERSION . ' </td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 3) {
                    ...
                    if ($view_downloads->getVar('size') != '') {
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_VIDEO_FORMSIZE . '</td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 4) {
                    ...
                    if ($view_downloads->getVar('platform') != '') {
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_VIDEO_FORMPLATFORM . ' </td>';
        ...
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMTEXT . ' </td>';
        ...
        if ($view_downloads->getVar('logourl') != 'blank.gif') {
            $class = ($class == 'even') ? 'odd' : 'even';
            echo '<tr class="' . $class . '">';
            echo '<td width="30%">' . _AM_VIDEO_FORMIMG . ' </td>';
            ...
            if ($view_downloads->getVar('type') == 'flv') {
                ...
                $xoTheme->addScript(XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname', 'n') . '/player/jwplayer-7/jwplayer.js');
            ...
            } elseif ($view_downloads->getVar('type') == 'ntv') {
                $xoTheme->addScript(XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname', 'n') . '/player/jwplayer-7/jwplayer.js');
                if (!empty($xoopsModuleConfig ['jwplayerKey'])) {
                    $jwplayerKey = "jwplayer.key='" . $xoopsModuleConfig ['jwplayerKey'] . "';";
                    $xoTheme->addScript(null, array('type' => 'text/javascript', 'charset' => _CHARSET), $jwplayerKey);
                ...
                $flashFile = $view_downloads->getVar('url') . $uploadpach_ntv . $view_downloads->getVar('filename');
                echo "<td><div id='mp4Player'></div>
					<script type='text/javascript'>jwplayer('mp4Player').setup({
        			file: '" . $flashFile . "',
        			image: '" . $imgurl . "',
        ...
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMDATE . ' </td>';
        echo '<td>' . formatTimestamp($view_downloads->getVar('date')) . '</td>';
        echo '</tr>';
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMPOSTER . ' </td>';
        echo '<td>' . XoopsUser::getUnameFromId($view_downloads->getVar('submitter')) . '</td>';
        echo '</tr>';
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMHITS . ' </td>';
        echo '<td>' . $view_downloads->getVar('hits') . '</td>';
        echo '</tr>';
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMRATING . ' </td>';
        echo '<td>' . number_format($view_downloads->getVar('rating'), 1) . ' (' . $view_downloads->getVar('votes') . ' ' . _AM_VIDEO_FORMVOTE . ')</td>';
        echo '</tr>';
        if ($view_downloads->getVar('paypal') != '' && $xoopsModuleConfig['use_paypal'] == true) {
            $class = ($class == 'even') ? 'odd' : 'even';
            echo '<tr class="' . $class . '">';
            echo '<td width="30%">' . _AM_VIDEO_FORMPAYPAL . ' </td>';
            echo '<td>' . $view_downloads->getVar('paypal') . '</td>';
            echo '</tr>';
        }
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMCOMMENTS . ' </td>';
        echo '<td>' . $view_downloads->getVar('comments') . ' <a href="../singlefile.php?cid=' . $view_downloads->getVar('cid') . '&lid=' . $downloads_lid . '"><img src="../images/icon/view_mini.png" alt="' . _AM_VIDEO_FORMDISPLAY . '" title="' . _AM_VIDEO_FORMDISPLAY . '"></a></td>';
        echo '</tr>';
        $class = ($class == 'even') ? 'odd' : 'even';
        echo '<tr class="' . $class . '">';
        echo '<td width="30%">' . _AM_VIDEO_FORMACTION . ' </td>';
        echo '<td>';
        echo($view_downloads->getVar('status') != 0 ? '' : '<a href="downloads.php?op=update_status&downloads_lid=' . $downloads_lid . '"><img src="./../images/icon/renew_mini.gif" border="0" alt="' . _AM_VIDEO_FORMVALID . '" title="' . _AM_VIDEO_FORMVALID . '"></a> ');
        echo '<a href="downloads.php?op=edit_downloads&downloads_lid=' . $downloads_lid . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_VIDEO_FORMEDIT .
            '" title="' . _AM_VIDEO_FORMEDIT . '"></a> <a href="downloads.php?op=del_downloads&downloads_lid=' . $downloads_lid . '">
        <img src="../images/icon/delete_mini.gif" alt="' . _AM_VIDEO_FORMDEL . '" title="' . _AM_VIDEO_FORMDEL . '"></a></td>';
        echo '</tr>';
        echo '</table>';
        echo '<br>';
        // Affichage des votes:
        ...
        printf(_AM_VIDEO_DOWNLOADS_VOTESUSER, $total_vote);
        echo '</b><br /><br /></td></tr>';
        echo '<tr><td><b>' . _AM_VIDEO_DOWNLOADS_VOTE_USER . '</b></td>' . '<td><b>' . _AM_VIDEO_DOWNLOADS_VOTE_IP . '</b></td>' . '<td align="center"><b>' . _AM_VIDEO_FORMRATING . '</b></td>'
            . '<td><b>' . _AM_VIDEO_FORMDATE . '</b></td>' . '<td align="center"><b>' . _AM_VIDEO_FORMDEL . '</b></td></tr>';
        ...
        printf(_AM_VIDEO_DOWNLOADS_VOTESANONYME, $total_vote);
        echo '</b><br /><br /></td></tr>';
        echo '<tr><td colspan="2"><b>' . _AM_VIDEO_DOWNLOADS_VOTE_IP . '</b></td>' . '<td align="center"><b>' . _AM_VIDEO_FORMRATING . '</b></td>'
            . '<td><b>' . _AM_VIDEO_FORMDATE . '</b></td>' . '<td align="center"><b>' . _AM_VIDEO_FORMDEL . '</b></td></tr>';
        foreach (array_keys($downloadsvotedata_arr) as $i) {
            echo '<tr>';
        ...
        if ($downloadsvotedata_Handler->delete($objvotedata)) {
            ...
            if ($total_vote == 0) {
                ...
                if ($downloads_Handler->insert($obj)) {
                    redirect_header('downloads.php?op=view_downloads&downloads_lid=' . $_REQUEST['lid'], 1, _AM_VIDEO_REDIRECT_DELOK);
                }
            } else {
                $total_rating = 0;
                foreach (array_keys($downloadsvotedata_arr) as $i) {
    ...
    case "save_downloads":
        ...
        if (isset($_POST['related'])) {
            $obj->setVar('related', video_checkrelated($_POST['related']));
        }
        ...
        if (intval($_REQUEST['size']) == 0) {
            if ($_REQUEST['size'] == '0' || $_REQUEST['size'] == '') {
                ...
                $erreur = true;
                $message_erreur .= _AM_VIDEO_ERREUR_SIZE . '<br>';
            }
        }
        ...
        if (isset($_REQUEST['description'])) {
            if ($_REQUEST['description'] == '') {
                $erreur = true;
                $message_erreur .= _AM_VIDEO_ERREUR_NODESCRIPTION . '<br>';
            }
        }
        ...
        if (isset($_REQUEST['cid'])) {
            if ($_REQUEST['cid'] == 0) {
                $erreur = true;
                $message_erreur .= _AM_VIDEO_ERREUR_NOCAT . '<br>';
            }
        }
        ...
        if ($erreur == 1) {
            ...
            // enregistrement
            if ($downloads_Handler->insert($obj)) {
                Video_Updateposts($donnee['submitter'], $donnee['status'], 'add');
                if (!isset($_REQUEST['downloads_modified'])) {
                ...
                if ($xoopsModuleConfig['permission_download'] == 2) {
                    ...
                    $criteria->add(new Criteria('gperm_name', 'video_download_item', '='));
                    $gperm_handler->deleteAll($criteria);
                    if (isset($_POST['item_download'])) {
                        foreach ($_POST['item_download'] as $onegroup_id) {
                            $gperm_handler->addRight('video_download_item', $lid_dowwnloads, $onegroup_id, $xoopsModule->getVar('mid'));
                        }
                    }
                }
                ...
                redirect_header('downloads.php', 2, _AM_VIDEO_REDIRECT_SAVE);
            }
            echo $obj->getHtmlErrors();
        }
    ...
    case "update_status":
        ...
        // Set msg
        $msg = sprintf(_AM_VIDEO_MAIL_TITLE, $submitter->getVar("uname"));
        $msg .= "\n\n" . sprintf(_AM_VIDEO_MAIL_MSG, formatTimestamp(time(), 's')) . "\n\n";
        $msg .= _AM_VIDEO_MAIL_URL . " : " . XOOPS_URL . '/modules/video/singlefile.php?lid=' . intval($_REQUEST['downloads_lid']) . "\n\n";
        ...
        $xoopsMailer->setSubject(_AM_VIDEO_MAIL_SUBJECT);
        ...
        if ($downloads_Handler->insert($obj) && $xoopsMailer->send()) {
            redirect_header('downloads.php?op=list&statut_display=0', 1, _AM_VIDEO_REDIRECT_SAVE);
        }
        echo $obj->getHtmlErrors();
        break;
    ...
    /*
     case "dbupdate":
       $criteria = new CriteriaCompo ();
         ...
         $video_id = '1';
       while ($video_id <= $last_id) {

            $obj = $downloads_Handler->get ( $video_id );
         if($obj) {

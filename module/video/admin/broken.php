...
if (video_checkModuleAdmin()){
		...
		$downloadsbroken_Handler->table_link = $downloadsbroken_Handler->db->prefix("video_downloads"); // Nom de la table en jointure
		...
		if ($numrows > 0) {
			...
			echo '<tr>';
			echo '<th align="center" width="10%">'._AM_VIDEO_FORMFILE.'</th>';
			...
			foreach (array_keys($downloadsbroken_arr) as $i) {
				...
				echo '<a href="downloads.php?op=view_downloads&downloads_lid=' . $downloads_lid . '"><img src="../images/icon/view_mini.png" alt="' . _AM_VIDEO_FORMDISPLAY . '" title="' . _AM_VIDEO_FORMDISPLAY . '"></a> ';
				echo '<a href="downloads.php?op=edit_downloads&downloads_lid=' . $downloads_lid . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_VIDEO_FORMEDIT . '" title="' . _AM_VIDEO_FORMEDIT . '"></a> ';
				echo '<a href="broken.php?op=del_brokendownloads&broken_id=' . $downloads_reportid . '"><img src="../images/icon/ignore_mini.png" alt="' . _AM_VIDEO_FORMIGNORE . '" title="' . _AM_VIDEO_FORMIGNORE . '"></a>';
				echo '</td>';
			}
			echo '</table><br />';
			echo '<br /><div align=right>' . $pagenav . '</div><br />';
		}else{
			echo '<div class="errorMsg" style="text-align: center;">' . _AM_VIDEO_ERREUR_NOBROKENDOWNLOADS . '</div>';
		}
		break;
	...
	case "del_brokendownloads":
		...
		if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
			...
			if ($downloadsbroken_Handler->delete($obj)) {
				redirect_header('broken.php', 1, _AM_VIDEO_REDIRECT_DELOK);
			}
...
if (video_checkModuleAdmin()){
                $broken_admin = new ModuleAdmin();
                $broken_admin->addItemButton(_MI_VIDEO_ADMENU4, 'broken.php', 'list');
                echo $broken_admin->renderButton();
            }
			xoops_confirm(array('ok' => 1, 'broken_id' => $_REQUEST['broken_id'], 'op' => 'del_brokendownloads'), $_SERVER['REQUEST_URI'], _AM_VIDEO_BROKEN_SURDEL . '<br>');
		}
		break;
}

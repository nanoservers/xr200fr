...
$xoopsOption ['template_main'] = 'video_liste.html';
...
//breadcrumb
$xoopsTpl->assign('breadcrumb', '<a href="' . XOOPS_URL . '">' . _MD_VIDEO_HOME . '</a>  &raquo; <a href="index.php">' . $xoopsModule->name() . '</a> &raquo; ' . _MD_VIDEO_SEARCH_BT);

//formulaire de recherche
$form = new XoopsThemeForm (_MD_VIDEO_SEARCH, "search", "search.php", 'post');
$form->setExtra('enctype="multipart/form-data"');

//recherche par titre
$form->addElement(new XoopsFormText (_MD_VIDEO_SEARCH_TITLE, 'title', 25, 255, $title));
...
/*$cat_select = new XoopsFormSelect(_MD_VIDEO_SEARCH_CATEGORIES . ' ', 'cat', $cat);
 $cat_select->addOption(0,_MD_VIDEO_SEARCH_ALL2);
 $cat_select->addOptionArray($downloadscat_Handler->getList($criteria ));
 $form->addElement($cat_select);*/

$downloadscat_arr = $downloadscat_Handler->getall($criteria);
$mytree = new XoopsObjectTree ($downloadscat_arr, 'cat_cid', 'cat_pid');
$form->addElement(new XoopsFormLabel (_AM_VIDEO_FORMINCAT, $mytree->makeSelBox('cat', 'cat_title', '--', $cat, true)));
...
foreach (array_keys($downloads_field) as $i) {
    ...
    if ($downloads_field [$i]->getVar('status_def') == 1) {
        ...
        if ($downloads_field [$i]->getVar('fid') == 1) {
            //page d'accueil
            $title_sup = _AM_VIDEO_FORMHOMEPAGE;
        ...
        if ($downloads_field [$i]->getVar('fid') == 2) {
            //version
            $title_sup = _AM_VIDEO_FORMVERSION;
        ...
        if ($downloads_field [$i]->getVar('fid') == 4) {
            //platform
            $title_sup = _AM_VIDEO_FORMPLATFORM;
            ...
            $criteria->setOrder('ASC');
            $video_arr = $downloads_Handler->getall($criteria);
            foreach (array_keys($video_arr) as $j) {
                $contenu_arr [$video_arr [$j]->getVar($nom_champ_base)] = $video_arr [$j]->getVar($nom_champ_base);
            }
            if ($champ_contenu [$downloads_field [$i]->getVar('fid')] != 999) {
        ...
        $criteria->setOrder('ASC');
        $video_arr = $downloadsfielddata_Handler->getall($criteria);
        foreach (array_keys($video_arr) as $j) {
            $contenu_arr [$video_arr [$j]->getVar('data', 'n')] = $video_arr [$j]->getVar('data');
        }
    ...
    $select_sup->addOption(999, _MD_VIDEO_SEARCH_ALL1);
...
$button_tray->addElement(new XoopsFormButton ('', 'submit', _MD_VIDEO_SEARCH_BT, 'submit'));
...
$video_arr = $downloads_Handler->getByLink($criteria_2);
if ($numrows > $limit) {
...
$xoopsTpl->assign('lang_thereare', sprintf(_MD_VIDEO_SEARCH_THEREARE, $downloads_Handler->getCount($criteria_2)));
$xoopsTpl->assign('pagenav', $pagenav);
$keywords = '';

foreach (array_keys($video_arr) as $i) {
    $video_tab ['lid'] = $video_arr [$i]->getVar('lid');
    $video_tab ['cid'] = $video_arr [$i]->getVar('cid');
    $video_tab ['title'] = $video_arr [$i]->getVar('title');
    $video_tab ['cat'] = $video_arr [$i]->getVar('cat_title');
    $video_tab ['imgurl'] = $uploadurl . $video_arr [$i]->getVar('cat_imgurl');
    $video_tab ['date'] = formatTimeStamp($video_arr [$i]->getVar('date'), 'd/m/Y');
    $video_tab ['rating'] = number_format($video_arr [$i]->getVar('rating'), 0);
    $video_tab ['hits'] = $video_arr [$i]->getVar('hits');
    $contenu = '';
    foreach (array_keys($downloads_field) as $j) {
        if ($downloads_field [$j]->getVar('status_def') == 1) {
            if ($downloads_field [$j]->getVar('fid') == 1) {
                //page d'accueil
                $contenu = $video_arr [$i]->getVar('homepage');
            }
            if ($downloads_field [$j]->getVar('fid') == 2) {
                //version
                $contenu = $video_arr [$i]->getVar('version');
            }
            if ($downloads_field [$j]->getVar('fid') == 3) {
                //taille du fichier
                $contenu = trans_size($video_arr [$i]->getVar('size'));
            }
            if ($downloads_field [$j]->getVar('fid') == 4) {
                //plateforme
                $contenu = $video_arr [$i]->getVar('platform');
            }
        } else {
            $criteria = new CriteriaCompo ();
            $criteria->add(new Criteria ('lid', $video_arr [$i]->getVar('lid')));
        ...
        $video_tab ['fielddata'] [$j] = $contenu;
        unset ($contenu);
    }
    $xoopsTpl->append('downloads', $video_tab);
    $keywords .= $video_arr [$i]->getVar('title') . ',';
}

$xoopsTpl->assign('searchForm', $form->render());
// r�f�rencement
// titre de la page
$titre = _MD_VIDEO_SEARCH_PAGETITLE . ' - ' . $xoopsModule->name();

...
include 'header.php';
// Page template
$xoopsOption ['template_main'] = 'audio_channel.html';
// Xoops Header
...
// Get User Id
$user = audio_CleanVars ( $_GET, 'user', 0, 'int' );
...
if(!$user) {
	...
	// Check user exist
	if(!$user) {
		redirect_header ( 'index.php', 3, _MD_AUDIO_CHANNEL_NOTSELECT );
		exit ();
	}
	...
	if(!$numrows) {
		redirect_header ( 'index.php', 3, _MD_AUDIO_CHANNEL_NOVIDEO);
	   exit ();
   }

   // breadcrumb
   $breadcrumb = _MD_AUDIO_CHANNEL_PAGENAVE . ' &raquo; ' . XoopsUser::getUnameFromId ( $user );	$xoopsTpl->assign('breadcrumb', $breadcrumb);
	$xoopsTpl->assign('breadcrumb', $breadcrumb);

   // Check use audio shot	if ($xoopsModuleConfig ['useshots'] == 1) {
...
// Set page title
$xoopsTpl->assign ( 'xoops_pagetitle', _MD_AUDIO_CHANNEL_PAGENAVE );

//description
$xoTheme->addMeta ( 'meta', 'description', _MD_AUDIO_CHANNEL_PAGENAVE );

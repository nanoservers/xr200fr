...
function b_audio_sel_show($options) {
	...
	foreach (array_keys($downloads_arr) as $i) {
		...
		$block[$i]['logourl'] = $downloads_arr[$i]->getVar('url') . '/uploads/audio/images/shots/' . $downloads_arr[$i]->getVar('logourl');
...
function b_audio_sel_edit($options) {
	//appel de la class
	...
	$form = _MB_AUDIO_HEIGHT . "&nbsp;\n";
	$form .= "<input type=\"hidden\" name=\"options[0]\" value=\"" . $options[0] . "\" />";
	$form .= "<input name=\"options[1]\" size=\"5\" maxlength=\"255\" value=\"" . $options[1] . "\" type=\"text\" />&nbsp;<br />";
	$form .= _MB_AUDIO_WIDTH . " : <input name=\"options[2]\" size=\"5\" maxlength=\"255\" value=\"" . $options[2] . "\" type=\"text\" /><br /><br />";

	array_shift($options);
	array_shift($options);
	array_shift($options);

	$form .= _MB_AUDIO_VIDTODISPLAY . "<br /><select name=\"options[]\" multiple=\"multiple\" size=\"5\">";
	//$form .= "<option value=\"0\" " . (array_search(0, $options) === false ? '' : 'selected="selected"') . ">" . _MB_AUDIO_ALLDOWNLAODS . "</option>";

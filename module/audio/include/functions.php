<?php
function audio_checkModuleAdmin()
{
...
function audio_MygetItemIds($permtype,$dirname)
{
	global $xoopsUser;
...
**/
function audio_NumbersOfEntries($mytree, $categories, $entries,$cid)
{
	$count = 0;
	$child_arr = array();
...
function audio_Thumbnail($time, $status) {
    ...
    if($xoopsModuleConfig['showupdated'] == 1) {
        if ($startdate < $time) {
            ...
            if ( !is_dir( XOOPS_ROOT_PATH . "/modules/audio/language/" . $language . "/" ) ){
                $language = 'english';
            }
            $img_path = XOOPS_ROOT_PATH . "/modules/audio/language/" . $language . "/images/";
            $img_url = XOOPS_URL . "/modules/audio/language/" . $language . "/images/";
            if($status==1) {
                if ( is_readable( $img_path . 'new.png') ){
                    $new = '&nbsp;<img src="' . $img_url . 'new.png" alt="" />';
                } else {
                    $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/audio/language/english/images/new.png" alt="" />';
                }
            }elseif($status==2) {
                if ( is_readable( $img_path . 'updated.png') ){                    
                    $new = '&nbsp;<img src="' . $img_url . 'updated.png" alt="" />';
                } else {
                    $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/audio/language/english/images/updated.png" alt="" />';
                }
...
function audio_Popular($hits) {
    ...
    if ($hits >= $xoopsModuleConfig['popular']) {
        ...
        if ( is_readable( $img_path . 'popular.png') ){
            ...
            $pop = '&nbsp;<img src ="' . XOOPS_URL . '/modules/audio/language/english/images/popular.png" alt="" />';
        }
    }
    return $pop;
}

function audio_trans_size($size) {
	if($size>0) {
		$mb = 1024*1024;
		if ( $size > $mb ) {
			$mysize = sprintf ("%01.2f",$size/$mb) . _AM_AUDIO_MB;
		}
		elseif ( $size >= 1024 ) {
			$mysize = sprintf ("%01.2f",$size/1024) . _AM_AUDIO_KB;
		}
		else {
			$mysize = sprintf(_AM_AUDIO_NUMBYTES,$size);
		}
		return $mysize;
	} else {
		return '';
...
function audio_formatTime($secs) {
	if($secs < 3600) {
		$times = array(60, 1);
		$time = '';
		$tmp = '';
...
function audio_CleanVars( &$global, $key, $default = '', $type = 'int' ) {
	switch ( $type ) {
		case 'string':
...
function audio_PathTree($mytree, $key, $category_array, $title, $prefix = '' )
{
...
function audio_PathTreeUrl($mytree, $key, $category_array, $title, $prefix = '', $link = false, $order = 'ASC', $lasturl = false)
{
	global $xoopsModule;
	$category_parent = $mytree->getAllParent($key);
	$Path = '<a href="' . XOOPS_URL . '">' . _MD_AUDIO_HOME . '</a>' . $prefix;
	if ($order == 'ASC'){
		$category_parent = array_reverse($category_parent);
		if ($link == true) {
...
function audio_checkrelated($inputs) {
	 ...
	 */
	function Audio_Updateposts($uid, $status, $action) {
		switch ($action) {
			case 'add' :
				if ($uid && $status) {

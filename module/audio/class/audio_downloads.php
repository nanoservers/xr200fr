...
class audio_downloads extends XoopsObject
{
	// constructor
	...
	function getForm($donnee = array(), $form_type, $erreur = false, $action = false)
	{
		...
		$title = $this->isNew() ? sprintf(_AM_AUDIO_FORMADD) : sprintf(_AM_AUDIO_FORMEDIT);

		//cr�ation du formulaire
		$form = new XoopsThemeForm($title, 'form', $action, 'post', true);
		$form->setExtra('enctype="multipart/form-data"');
		//titre
		$form->addElement(new XoopsFormText(_AM_AUDIO_FORMTITLE, 'title', 50, 255, $this->getVar('title')), true);
		// fichier
		$fichier = new XoopsFormElementTray(_AM_AUDIO_FORMFILE,'<br /><br />');
      if($this->isNew()) {
	      switch($form_type) {
		      case 'copy':
			      $url = $this->isNew() ? '' : $this->getVar('url');
					$formmp3 = new XoopsFormText(_AM_AUDIO_FORM_FLVURL, 'mp3Url', 75, 255, $this->getVar('filename'));
					$fichier->addElement($formmp3,true);
					$formurl = new XoopsFormText(_AM_AUDIO_FORMURL, 'url', 75, 255, $url);
					$fichier->addElement($formurl,true);
	      		$audio_type = 'copy';
		      break;
		      
		      case 'upload':
					$fichier->addElement(new XoopsFormFile(_AM_AUDIO_FORMUPLOAD , 'attachedfile', $xoopsModuleConfig['maxuploadsize']), true);
				   $audio_type = 'upload';
		      break;
      ...
      $form->addElement(new XoopsFormHidden('audio_type', $audio_type));
		//cat�gorie
		$downloadscat_Handler =& xoops_getModuleHandler('audio_cat', 'audio');
		$categories = audio_MygetItemIds('audio_submit', 'audio');
		...
		$form->addElement(new XoopsFormLabel(_AM_AUDIO_FORMINCAT, $mytree->makeSelBox('cid', 'cat_title','--',$this->getVar('cid'),false)), true);
		 
		if ($xoopsUser->isAdmin($xoopsModule->mid()) ) {
			$form->addElement ( new XoopsFormRadioYN ( _AM_AUDIO_TOP, 'top', $this->getVar ( 'top', 'e' ) ) );
		} 
		 		
		//titre
		//$form->addElement(new XoopsFormText(_AM_AUDIO_DURATION , 'duration', 10, 50, $this->getVar('duration')), false);
		...
		foreach (array_keys($downloads_field) as $i) {
			if ($downloads_field[$i]->getVar('status_def') == 1){
				if ($downloads_field[$i]->getVar('fid') == 1){
					...
					if ($downloads_field[$i]->getVar('status') == 1){
						$form->addElement(new XoopsFormText(_AM_AUDIO_FORMHOMEPAGE, 'homepage', 50, 255, $this->getVar('homepage')));
					} else {
				...
				if ($downloads_field[$i]->getVar('fid') == 2){
					//version
					if ($downloads_field[$i]->getVar('status') == 1){
						$form->addElement(new XoopsFormText(_AM_AUDIO_FORMVERSION, 'version', 10, 255, $this->getVar('version')));
					} else {
				...
				if ($downloads_field[$i]->getVar('fid') == 3){
					//version
					if ($downloads_field[$i]->getVar('status') == 1){
						$form->addElement(new XoopsFormText(_AM_AUDIO_FORMSIZE, 'size', 10, 50, $this->getVar('size')));
					} else {
					...
					if ($downloads_field[$i]->getVar('status') == 1){
						...
						$aff_size = new XoopsFormElementTray(_AM_AUDIO_FORMSIZE,'');
				...
				if ($downloads_field[$i]->getVar('fid') == 4){
					//duration
					if ($downloads_field[$i]->getVar('status') == 1){
						$form->addElement(new XoopsFormText(_AM_AUDIO_DURATION, 'duration', 10, 50, $this->getVar('duration')));
					} else {
           ...
           if ($downloads_field[$i]->getVar('fid') == 5){
					//plateforme
					if ($downloads_field[$i]->getVar('status') == 1){
						$platformselect = new XoopsFormSelect(_AM_AUDIO_FORMPLATFORM, 'platform', explode('|',$this->getVar('platform')), 5, true);
		...
		$form->addElement( new XoopsFormEditor(_AM_AUDIO_FORMTEXTDOWNLOADS, "description", $editor_configs), true);
		//extra
		if ($xoopsModuleConfig['shwo_extra'] == 1) {
			$form->addElement( new XoopsFormEditor(_AM_AUDIO_FORMEXTRATEXT, "extra", $editor_configs), false);
		}
		//tag
		...
		//image
		$uploaddir = XOOPS_ROOT_PATH . '/uploads/audio/images/shots/' . $this->getVar('logourl');
		$downloadscat_img = $this->getVar('logourl') ? $this->getVar('logourl') : 'blank.gif';
		if (!is_file($uploaddir)){
			$downloadscat_img = 'blank.gif';
		}
		$uploadirectory='/uploads/audio/images/shots';
		$imgtray = new XoopsFormElementTray(_AM_AUDIO_FORMIMG,'<br />');
		if(!$this->isNew()) {
		...
		if ($perm_upload == true) {
			$fileseltray->addElement(new XoopsFormFile(_AM_AUDIO_FORMUPLOAD , 'attachedimage', $xoopsModuleConfig['maxuploadsize']), false);
		}
		$imgtray->addElement($fileseltray);
		$imgtray->addElement(new XoopsFormText(_AM_AUDIO_FORMIMG, 'logourl', 50, 255, $this->getVar('logourl')), false);
		if(!$this->isNew()) {
		   $newimg = new XoopsFormCheckBox('', 'newimg', '0');
		   $newimg->addOption(1, _AM_AUDIO_NEWIMG);
		...
		if ($xoopsUser) {
			if ( $xoopsUser->isAdmin($xoopsModule->mid()) ) {
				...
				$form->addElement(new XoopsFormSelectUser(_AM_AUDIO_FORMSUBMITTER, 'submitter', true, $submitter, 1, false), true);
				// date
				if (!$this->isNew()) {
					$selection_date = new XoopsFormElementTray(_AM_AUDIO_FORMDATEUPDATE);
					$date = new XoopsFormRadio('', 'date_update', $date_update);
					$options = array('N' =>_AM_AUDIO_FORMDATEUPDATE_NO . ' (' . formatTimestamp($v_date,'s') . ')', 'Y' => _AM_AUDIO_FORMDATEUPDATE_YES);
				...
				$status = new XoopsFormCheckBox(_AM_AUDIO_FORMSTATUS, 'status', $v_status);
				$status->addOption(1, _AM_AUDIO_FORMSTATUS_OK);
				...
				if ($xoopsModuleConfig['permission_download'] == 2) {
					...
					global $xoopsModule;
					if(!$this->isNew()) {
						$item_ids_download = $gperm_handler->getGroupIds('audio_download_item', $this->getVar('lid'), $xoopsModule->getVar('mid'));
						$item_ids_downloa = array_values($item_ids_download);
						$item_news_can_download_checkbox = new XoopsFormCheckBox(_AM_AUDIO_PERM_DOWNLOAD_DSC, 'item_download[]', $item_ids_download);
					} else {
      ...
      //related
      if ($xoopsModuleConfig['audio_related'] == true){
	      $form->addElement(new XoopsFormText(_AM_AUDIO_FORMRELATED, 'related', 50, 255, $this->getVar('related')), false);
		}
		//paypal
		if ($xoopsModuleConfig['use_paypal'] == true){
			$form->addElement(new XoopsFormText(_AM_AUDIO_FORMPAYPAL, 'paypal', 50, 255, $this->getVar('paypal')), false);
		}
		// captcha
	...
	function FormOne($uuid, $cid) {
		...
		//cr�ation du formulaire
		$form = new XoopsThemeForm(_AM_AUDIO_FORMADD, 'form', 'submit.php', 'post', true);
		$form->setExtra('enctype="multipart/form-data"');
		// audio
		if ($perm_upload == true) {
			$audio_upload = new XoopsFormFile (_AM_AUDIO_FORMUPLOAD, 'attachedfile', $xoopsModuleConfig['maxuploadsize']);
			$audio = new XoopsFormElementTray ( _AM_AUDIO_FORMFILE );
			$audio->addElement ( $audio_upload );
			$form->addElement ($audio);
	...
	function FormTwo() {
		global $xoopsDB, $xoopsModule, $xoopsModuleConfig, $xoopsUser;
		//cr�ation du formulaire
		$form = new XoopsThemeForm(_AM_AUDIO_FORMADD, 'form', $action = false, 'post', true);
		$form->setExtra('enctype="multipart/form-data"');
		//titre
		$form->addElement(new XoopsFormText(_AM_AUDIO_FORMTITLE, 'title', 50, 255, $this->getVar('title')), true);
		...
		//image
		$imgtray = new XoopsFormElementTray(_AM_AUDIO_FORMIMG,'<br />');
		$imgtray->addElement(new XoopsFormFile(_AM_AUDIO_FORMUPLOAD , 'attachedimage', $xoopsModuleConfig['maxuploadsize']), false);
		$form->addElement($imgtray);
		...
		if ($xoopsUser) {
			if ( $xoopsUser->isAdmin($xoopsModule->mid()) ) {
				$status = new XoopsFormCheckBox(_AM_AUDIO_FORMSTATUS, 'status', 0);
				$status->addOption(1, _AM_AUDIO_FORMSTATUS_OK);
				$form->addElement($status);
			}
		}
...
class audioaudio_downloadsHandler extends XoopsPersistableObjectHandler
{
	function __construct(&$db)
	{
		parent::__construct($db, "audio_downloads", 'audio_downloads', 'lid', 'title');
	}
}
?>
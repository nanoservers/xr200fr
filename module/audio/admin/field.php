...
switch ($op)
{
	...
	case "list":
		...
		if (audio_checkModuleAdmin()){
            ...
            $field_admin->addItemButton(_AM_AUDIO_FIELD_NEW, 'field.php?op=new_field', 'add');
            $field_admin->addItemButton(_AM_AUDIO_FIELD_LIST, 'field.php?op=list', 'list');
		...
		if ($numrows>0) {
			...
			echo '<tr>';
			echo '<th align="left">' . _AM_AUDIO_FORMTITLE . '</th>';
			echo '<th align="center" width="10%">' . _AM_AUDIO_FORMIMAGE . '</th>';
			echo '<th align="center" width="10%">' . _AM_AUDIO_FORMWEIGHT . '</th>';
			echo '<th align="center" width="10%">' . _AM_AUDIO_FORMAFFICHE . '</th>';
			echo '<th align="center" width="10%">' . _AM_AUDIO_FORMAFFICHESEARCH . '</th>';
			echo '<th align="center" width="10%">' . _AM_AUDIO_FORMACTION . '</th>';
			echo '</tr>';
			$class = 'odd';
			foreach (array_keys($downloads_field) as $i) {
				...
				echo '<td align="center" width="10%">';
				echo '<a href="field.php?op=edit_field&fid=' . $downloadsfield_fid . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_AUDIO_FORMEDIT . '" title="' . _AM_AUDIO_FORMEDIT . '"></a> ';
				if ($downloads_field[$i]->getVar('status_def') == 0){
					echo '<a href="field.php?op=del_field&fid=' . $downloadsfield_fid . '"><img src="../images/icon/delete_mini.gif" alt="' . _AM_AUDIO_FORMDEL . '" title="' . _AM_AUDIO_FORMDEL . '"></a>';
				}
				echo '</td>';
				echo '</tr>';
	...
	case "update_status":
		...
		if ($downloadsfield_Handler->insert($obj)) {
			redirect_header('field.php?op=list', 1, _AM_AUDIO_REDIRECT_SAVE);
		}
	...
	case "del_field":
		...
		if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
			...
			if ($downloadsfield_Handler->delete($obj)) {
				redirect_header('field.php', 1, _AM_AUDIO_REDIRECT_DELOK);
			} else {
				echo $obj->getHtmlErrors();
			}
		} else {
			...
			if ($downloadsfield->getVar('status_def') == 1){
				redirect_header('field.php', 2, _AM_AUDIO_REDIRECT_NODELFIELD);
			}
			$message = '';
			$criteria = new CriteriaCompo();
			$criteria->add(new Criteria('fid', $_REQUEST['fid']));
			...
			xoops_confirm(array('ok' => 1, 'fid' => $_REQUEST['fid'], 'op' => 'del_field'), $_SERVER['REQUEST_URI'], sprintf(_AM_AUDIO_FORMSUREDEL, $obj->getVar('title')) . '<br><br>' . $message);
		}
	...
	case "save_field":
		...
		if ($erreur==true){
			...
			if ($downloadsfield_Handler->insert($obj)) {
				redirect_header('field.php', 1, _AM_AUDIO_REDIRECT_SAVE);
			}
			echo $obj->getHtmlErrors();
		}
		$form =& $obj->getForm();

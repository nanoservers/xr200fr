...
switch ($op)
{
	...
	case "list":
		...
		if (audio_checkModuleAdmin()){
            ...
            $category_admin->addItemButton(_AM_AUDIO_CAT_NEW, 'category.php?op=new_cat', 'add');
		...
		if (count($downloads_cat)>0) {
			...
			echo '<tr>';
			echo '<th align="left" width="25%">' . _AM_AUDIO_FORMTITLE . '</th>';
			echo '<th align="center">' . _AM_AUDIO_FORMTEXT . '</th>';
			echo '<th align="center" width="3%">' . _AM_AUDIO_FORMWEIGHT . '</th>';
			echo '<th align="center" width="5%">' . _AM_AUDIO_FORMACTION . '</th>';
			echo '</tr>';
			$class = 'odd';
			include_once XOOPS_ROOT_PATH."/modules/audio/class/tree.php";
			$mytree = new ObjectTree($downloads_cat, 'cat_cid', 'cat_pid');
			...
			foreach (array_keys($category_ArrayTree) as $i) {
				echo '<tr class="'.$class.'">';
				echo '<td align="left" ><a href="' . XOOPS_URL . '/modules/audio/viewcat.php?cid=' . $i . '">' . $category_ArrayTree[$i] . '</a></td>';
				...
				echo '<a href="category.php?op=edit_cat&downloadscat_cid=' . $i . '"><img src="../images/icon/edit_mini.gif" alt="'._AM_AUDIO_FORMEDIT.'" title="'._AM_AUDIO_FORMEDIT.'"></a> ';
				echo '<a href="category.php?op=del_cat&downloadscat_cid=' . $i . '"><img src="../images/icon/delete_mini.gif" alt="'._AM_AUDIO_FORMDEL.'" title="'._AM_AUDIO_FORMDEL.'"></a>';
				echo '</td>';
				echo '</tr>';
	...
	case "new_cat":
		...
		if (audio_checkModuleAdmin()){
            $category_admin = new ModuleAdmin();
            echo $category_admin->addNavigation('category.php');
            $category_admin->addItemButton(_AM_AUDIO_CAT_LIST, 'category.php?op=list', 'list');
            echo $category_admin->renderButton();
        }
	...
	case "edit_cat":
		...
		//Affichage du formulaire de création des catégories
		$downloadscat_cid = audio_CleanVars($_REQUEST, 'downloadscat_cid', 0, 'int');
	...
	case "del_cat":
		...
		if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
			...
			if ($downloadscat_Handler->delete($obj)) {
				redirect_header('category.php', 1, _AM_AUDIO_REDIRECT_DELOK);
			} else {
			...
			if (count($downloads_arr) > 0) {
				$message .= _AM_AUDIO_DELDOWNLOADS .'<br>';
				foreach (array_keys($downloads_arr) as $i) {
			...
			if (count($downloads_childcat) > 0) {
				$message .=_AM_AUDIO_DELSOUSCAT . ' <br><br>';
				foreach (array_keys($downloads_childcat) as $i) {
			...
			xoops_confirm(array('ok' => 1, 'downloadscat_cid' => $downloadscat_cid, 'op' => 'del_cat'), $_SERVER['REQUEST_URI'], sprintf(_AM_AUDIO_FORMSUREDEL, $obj->getVar('cat_title')) . '<br><br>' . $message);
		}
	...
	case "save_cat":
		...
		$cat_cid = audio_CleanVars($_REQUEST, 'cat_cid', 0, 'int');
		...
		$obj->setVar('cat_tab', $_POST["cat_tab"]);
		if (intval($_REQUEST['cat_weight'])==0 && $_REQUEST['cat_weight'] != '0'){
			$erreur=true;
			$message_erreur = _AM_AUDIO_ERREUR_WEIGHT . '<br>';
		}
		if (isset($_REQUEST['cat_cid'])){
			if ($cat_cid == $_REQUEST['cat_pid']){
				$erreur=true;
				$message_erreur .= _AM_AUDIO_ERREUR_CAT;
			}
		}
		if ($erreur==true){
			...
			if ($downloadscat_Handler->insert($obj)) {
				...
				$criteria->add(new Criteria('gperm_name', 'audio_view', '='));
				$gperm_handler->deleteAll($criteria);
				if(isset($_POST['groups_view'])) {
					foreach($_POST['groups_view'] as $onegroup_id) {
						$gperm_handler->addRight('audio_view', $perm_id, $onegroup_id, $xoopsModule->getVar('mid'));
					}
				}
				...
				$criteria->add(new Criteria('gperm_itemid', $perm_id, '='));
				$criteria->add(new Criteria('gperm_modid', $xoopsModule->getVar('mid'),'='));
				$criteria->add(new Criteria('gperm_name', 'audio_submit', '='));
				$gperm_handler->deleteAll($criteria);
				if(isset($_POST['groups_submit'])) {
					foreach($_POST['groups_submit'] as $onegroup_id) {
						$gperm_handler->addRight('audio_submit', $perm_id, $onegroup_id, $xoopsModule->getVar('mid'));
					}
				}
				...
				if ($xoopsModuleConfig['permission_download'] == 1) {
					...
					$criteria->add(new Criteria('gperm_itemid', $perm_id, '='));
					$criteria->add(new Criteria('gperm_modid', $xoopsModule->getVar('mid'),'='));
					$criteria->add(new Criteria('gperm_name', 'audio_download', '='));
					$gperm_handler->deleteAll($criteria);
					if(isset($_POST['groups_download'])) {
						foreach($_POST['groups_download'] as $onegroup_id) {
							$gperm_handler->addRight('audio_download', $perm_id, $onegroup_id, $xoopsModule->getVar('mid'));
						}
					}
				}
				//notification
				...
				redirect_header('category.php?op=list', 1, _AM_AUDIO_REDIRECT_SAVE);
			}
			echo $obj->getHtmlErrors();
		}
		$form =& $obj->getForm();

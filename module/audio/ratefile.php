...
$xoopsOption ['template_main'] = 'audio_ratefile.html';
...
switch ($op)
{
	// Vue liste	case "liste" :
		...
		$breadcrumb .= ' &raquo; ' . _MD_AUDIO_SINGLEFILE_RATHFILE;
		$xoopsTpl->assign ( 'breadcrumb', $breadcrumb );
		// r�f�rencement		// titre de la page		$pagetitle = _MD_AUDIO_SINGLEFILE_RATHFILE . ' - ' . $view_downloads->getVar ( 'title' ) . ' - ';
		$pagetitle .= audio_PathTreeUrl ( $mytree, $view_downloads->getVar ( 'cid' ), $downloadscat_arr, 'cat_title', $prefix = ' - ', false, 'DESC', true );
		$xoopsTpl->assign ( 'xoops_pagetitle', $pagetitle );
		//description		$xoTheme->addMeta ( 'meta', 'description', strip_tags ( _MD_AUDIO_SINGLEFILE_RATHFILE . ' (' . $view_downloads->getVar ( 'title' ) . ')' ) );
		...
		// si c'est un membre on v�rifie qu'il ne vote pas pour son fichier		if ($ratinguser != 0) {
			...
			foreach ( array_keys ( $downloads_arr ) as $i ) {
				if ($downloads_arr [$i]->getVar ( 'submitter' ) == $ratinguser) {
					redirect_header ( 'singlefile.php?lid=' . intval ( $_REQUEST ['lid'] ), 2, _MD_AUDIO_RATEFILE_CANTVOTEOWN );
					exit ();
				}
			}
			...
			foreach ( array_keys ( $downloadsvotes_arr ) as $i ) {
				if ($downloadsvotes_arr [$i]->getVar ( 'ratinguser' ) == $ratinguser) {
					redirect_header ( 'singlefile.php?lid=' . intval ( $_REQUEST ['lid'] ), 2, _MD_AUDIO_RATEFILE_VOTEONCE );
					exit ();
				}
			}
		} else {
		...
		if ($rating < 0 || $rating > 10) {
			$message_erreur .= _MD_AUDIO_RATEFILE_NORATING . '<br>';
			$erreur = true;
		}
		xoops_load ( "captcha" );
		...
		if ($erreur == true) {
			...
			if ($downloadsvotedata_Handler->insert ( $obj )) {
				...
				if ($downloads_Handler->insert ( $objdownloads )) {
					redirect_header ( 'singlefile.php?lid=' . $lid, 2, _MD_AUDIO_RATEFILE_VOTEOK );
				}

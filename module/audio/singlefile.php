...
$xoopsOption ['template_main'] = 'audio_singlefile.html';
...
if (is_object ( $xoopsUser ) && $xoopsUser->isAdmin ( $xoopsModule->mid () )) {
	$adminlink = '<a href="' . XOOPS_URL . '/modules/audio/admin/downloads.php?op=view_downloads&amp;downloads_lid=' . $_REQUEST ['lid'] . '" title="' . _MD_AUDIO_EDITTHISDL . '"><img src="' . XOOPS_URL . '/modules/audio/images/editicon.png" width="16px" height="16px" border="0" alt="' . _MD_AUDIO_EDITTHISDL . '" /></a>';
} else {
	$adminlink = '';
}
...
$audiocount = $downloads_Handler->getCount ( $criteria );

// build information
$new = audio_Thumbnail ( $view_downloads->getVar ( 'date' ), $view_downloads->getVar ( 'status' ) );
$pop = audio_Popular ( $view_downloads->getVar ( 'hits' ) );
...
$xoopsTpl->assign ( 'mp3Height', $xoopsModuleConfig ['playerHeight'] );
$xoopsTpl->assign ( 'mp3Width', $xoopsModuleConfig ['playerWidth'] );
...
$xoopsTpl->assign ( 'audioFile', $view_downloads->getVar ( 'url' ) . $uploadpach_mp3 . $view_downloads->getVar ( 'filename' ) . '.mp3' );
...
$xoopsTpl->assign ( 'audiocount', $audiocount );
$xoopsTpl->assign ( 'hits', sprintf ( _MD_AUDIO_SINGLEFILE_NBTELECH, $view_downloads->getVar ( 'hits' ) ) );
$xoopsTpl->assign ( 'rating', number_format ( $view_downloads->getVar ( 'rating' ), 1 ) );
$xoopsTpl->assign ( 'votes', sprintf ( _MD_AUDIO_SINGLEFILE_VOTES, $view_downloads->getVar ( 'votes' ) ) );
$xoopsTpl->assign ( 'nb_comments', sprintf ( _MD_AUDIO_SINGLEFILE_COMMENTS, $view_downloads->getVar ( 'comments' ) ) );
	...
	if ($downloads_field [$i]->getVar ( 'status_def' ) == 1) {
		if ($downloads_field [$i]->getVar ( 'fid' ) == 1) {
			...
			if ($view_downloads->getVar ( 'homepage' ) != '') {
				$champ_sup = '&nbsp;' . _AM_AUDIO_FORMHOMEPAGE . ':&nbsp;<a href="' . $view_downloads->getVar ( 'homepage' ) . '">' . _MD_AUDIO_SINGLEFILE_ICI . '</a>';
				$champ_sup_vide ++;
			}
		}
		if ($downloads_field [$i]->getVar ( 'fid' ) == 2) {
			...
			if ($view_downloads->getVar ( 'version' ) != '') {
				$champ_sup = '&nbsp;' . _AM_AUDIO_FORMVERSION . ':&nbsp;' . $view_downloads->getVar ( 'version' );
				$champ_sup_vide ++;
			}
		}
		if ($downloads_field [$i]->getVar ( 'fid' ) == 3) {
			...
			if ($view_downloads->getVar ( 'size' ) != '') {
				$champ_sup = '&nbsp;' . _AM_AUDIO_FORMSIZE . ':&nbsp;' . audio_trans_size($view_downloads->getVar ( 'size' ));
				$champ_sup_vide ++;
			}
		}
		if ($downloads_field [$i]->getVar ( 'fid' ) == 4) {
			...
			if ($view_downloads->getVar ( 'duration' ) != '') {
				$champ_sup = '&nbsp;' . _AM_AUDIO_DURATION . ':&nbsp;' . audio_formatTime($view_downloads->getVar ( 'duration' ));
				$champ_sup_vide ++;
			}
		}
		if ($downloads_field [$i]->getVar ( 'fid' ) == 5) {
			...
			if ($view_downloads->getVar ( 'platform' ) != '') {
				$champ_sup = '&nbsp;' . _AM_AUDIO_FORMPLATFORM . ':&nbsp;' . $view_downloads->getVar ( 'platform' );
				$champ_sup_vide ++;
			}
		}
		$contenu = '';
...
if (($xoopsModuleConfig ['usetellafriend'] == 1) and (is_dir ( '../tellafriend' ))) {
	$string = sprintf ( _MD_AUDIO_SINGLEFILE_INTFILEFOUND, $xoopsConfig ['sitename'] . ':  ' . XOOPS_URL . '/modules/audio/singlefile.php?lid=' . $_REQUEST ['lid'] );
	$subject = sprintf ( _MD_AUDIO_SINGLEFILE_INTFILEFOUND, $xoopsConfig ['sitename'] );
	if (stristr ( $subject, '%' ))
	...
	$tellafriend_texte = '<a target="_top" href="' . XOOPS_URL . '/modules/tellafriend/index.php?target_uri=' . rawurlencode ( $target_uri ) . '&amp;subject=' . rawurlencode ( $subject ) . '">' . _MD_AUDIO_SINGLEFILE_TELLAFRIEND . '</a>';
} else {
	$tellafriend_texte = '<a target="_top" href="mailto:?subject=' . rawurlencode ( sprintf ( _MD_AUDIO_SINGLEFILE_INTFILEFOUND, $xoopsConfig ['sitename'] ) ) . '&amp;body=' . rawurlencode ( sprintf ( _MD_AUDIO_SINGLEFILE_INTFILEFOUND, $xoopsConfig ['sitename'] ) . ':  ' . XOOPS_URL . '/modules/audio/singlefile.php?lid=' . $_REQUEST ['lid'] ) . '">' . _MD_AUDIO_SINGLEFILE_TELLAFRIEND . '</a>';
}
...
$xoopsTpl->assign ( 'audioblock', $related );
$xoopsTpl->assign ( 'showaudioblock', 1 );
